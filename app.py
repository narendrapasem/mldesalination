import streamlit as st
import pickle as pt
import pandas as pd
import numpy as np
from PIL import Image

model = pt.load(open('model.sav', 'rb'))

st.title('Prediction of Hourly Productivity')
st.sidebar.header('Input Parameters')
image = Image.open('img.jpg')
st.image(image,'')

def user_report():
  T_water = st.sidebar.slider('Water Temperature',20,70,1)
  T_inside_glass = st.sidebar.slider('Inside Glass Temperature',30,70,1)
  Ambient_Temperature = st.sidebar.slider('Ambient Temperature',20,40,1)
  Irradiance = st.sidebar.slider('Irradiance',0,1000,1)
  Mirror = st.sidebar.slider('Mirror',0,3,0)

  user_report_data = {
    'T_water': T_water,
    'T_inside_glass': T_inside_glass,
    'Ambient_Temperature': Ambient_Temperature,
    'Irradiance': Irradiance,
    'Mirror': Mirror  
  }

  report_data = pd.DataFrame(user_report_data, index=[0])
  return report_data

user_data = user_report()
st.header('Input Parameters')
st.write(user_data)

hourly_productivity = model.predict(user_data)
st.subheader('Hourly Productivity')
st.subheader(str(np.round(hourly_productivity[0], 4)) + ' Kg/m2')